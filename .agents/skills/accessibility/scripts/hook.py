...
que les fichiers de `dsmaccess/Views/`, et seulement celui qui vient de changer :
auditer tout le diff redirait les mêmes constats à chaque édition d'une série.
    ...
    print(json.dumps({
        ...
        "hookSpecificOutput": {
            ...
            "additionalContext": (
                ...
                "Corriger avant de dire l'écran fini. Un constat `regions` sur un "
                "`List` ou un `ScrollView` peut être un faux positif : lire les "
                "modificateurs à la fermeture du bloc avant de poser un libellé."
            ),
        },
    }))
    return 0
